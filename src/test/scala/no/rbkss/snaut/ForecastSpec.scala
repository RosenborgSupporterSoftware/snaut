...
class ForecastSpec extends FlatSpec with Matchers {
    "Forecast" should "load forecasts from file" in {
        var f = new Forecast().loadURL(getClass.getResource("/lerkendal_varsel.xml"))
        var g = new Forecast().loadURL(getClass.getResource("/lerkendal_varsel_time_for_time.xml"))
    }
